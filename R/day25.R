...
#'
#' @param x some data
#' @return For Part One, `f25a_find_stable_grid(x)` returns a list with the
#'   final steady state grid and the number of steps required to reach that
#'   state.
#' @export
#' @examples
#' f25a_find_stable_grid(example_data_25())
f25a_find_stable_grid <- function(x) {
  # strategy: matrix subsetting
  grid_prev <- f25_prepare_grid(x)
  grid_next <- f25_step_once(grid_prev)
  n <- 1

  while (any(grid_prev != grid_next)) {
    n <- n + 1
    grid_prev <- grid_next
    grid_next <- f25_step_once(grid_next)
  }
...
f25_step_once <- function(grid) {
  nc <- ncol(grid)
  nr <- nrow(grid)
  rows_wrap <- seq(2, nr + 1) |> mod1(nr)
  cols_wrap <- seq(2, nc + 1) |> mod1(nc)

  # east facing & empty eastward spot
  easties <- which(grid[] == ">" & grid[, cols_wrap] == ".", arr.ind = TRUE)
  easties2 <- easties
  easties2[, "col"] <- mod1(easties2[, "col"] + 1, nc)
  grid[easties] <- "."
  grid[easties2] <- ">"

  # south facing & empty southward spot
  southies <- which(grid[] == "v" & grid[rows_wrap, ] == ".", arr.ind = TRUE)
  southies2 <- southies
  southies2[, "row"] <- mod1(southies2[, "row"] + 1, nr)
  grid[southies] <- "."
  grid[southies2] <- "v"
  grid
}
